...
final class StubHttpClient implements HttpClientInterface
{
    ...
    {
        if ('GET' !== $method) {
            throw new \LogicException('Not implemented: NZ Companies Office API client should only use GET requests');
        }
    ...
    {
        throw new \LogicException('Not implemented: NZ Companies Office API client should only be using the request method');
    }
    ...
    private function getMockHttpClient(): MockHttpClient
    {
        ...
        return new MockHttpClient($this->stubResponse, 'https://api.business.govt.nz/gateway/nzbn/v5/');
    }
}
